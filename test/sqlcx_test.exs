defmodule Sqlcx.Test do
  ...
  setup_all do
    ...
    {:ok, golf_db: Sqlcx.TestDatabase.init(db)}
  end

  test "set_update_hook", %{golf_db: db} do
    :ok = Sqlcx.set_update_hook(db, self())
    :ok = Sqlcx.exec(db, "CREATE TABLE test (id INTEGER PRIMARY KEY, val STRING);")
    :ok = Sqlcx.exec(db, "INSERT INTO test (val) VALUES ('this is a test');")
    assert_receive {:insert, 'test', 1}

    {:ok, conn} = Sqlcx.Server.start_link(@shared_cache)
    :ok = Sqlcx.Server.set_update_hook(conn, self())
  ...
  test "a basic query returns a list of keyword lists", context do
    {:ok, [row]} = Sqlcx.query(context[:golf_db], "SELECT * FROM players ORDER BY id LIMIT 1")
    assert row == [id: 1, name: "Mikey", created_at: {{2012, 10, 14}, {05, 46, 28, 318_107}}, updated_at: {{2013, 09, 06}, {22, 29, 36, 610_911}}, type: nil]
  end

  test "a basic query returns a list of maps when into: %{} is given", context do
    {:ok, [row]} = Sqlcx.query(context[:golf_db], "SELECT * FROM players ORDER BY id LIMIT 1", into: %{})
    assert row == %{id: 1, name: "Mikey", created_at: {{2012, 10, 14}, {05, 46, 28, 318_107}}, updated_at: {{2013, 09, 06}, {22, 29, 36, 610_911}}, type: nil}
  end
  ...
  test "a parameterized query", context do
    {:ok, [row]} = Sqlcx.query(context[:golf_db], "SELECT id, name FROM players WHERE name LIKE ?1 AND type == ?2", bind: ["s%", "Team"])
    assert row == [id: 25, name: "Slothstronauts"]
  end

  test "a parameterized query into %{}", context do
    {:ok, [row]} = Sqlcx.query(context[:golf_db], "SELECT id, name FROM players WHERE name LIKE ?1 AND type == ?2", bind: ["s%", "Team"], into: %{})
    assert row == %{id: 25, name: "Slothstronauts"}
  end
  ...
  test "query_rows returns {:ok, data}", context do
    {:ok, result} = Sqlcx.query_rows(context[:golf_db], "SELECT id, name FROM players WHERE name LIKE ?1 AND type == ?2", bind: ["s%", "Team"])
  ...
  test "query_rows return {:error, reason}", context do
    {:error, reason} = Sqlcx.query_rows(context[:golf_db], "SELECT wat FROM players")
    assert reason == {:sqlite_error, 'no such column: wat'}
  end

  test "query_rows! returns data", context do
    result = Sqlcx.query_rows!(context[:golf_db], "SELECT id, name FROM players WHERE name LIKE ?1 AND type == ?2", bind: ["s%", "Team"])
    %{rows: rows, columns: columns, types: types} = result
    assert rows == [[25, "Slothstronauts"]]
    assert columns == [:id, :name]
    assert types == [:INTEGER, :"varchar(255)"]
  ...
  test "query_rows! raises on error", context do
    assert_raise Sqlcx.QueryError, "Query failed: {:sqlite_error, 'no such column: wat'}", fn ->
      [_res] = Sqlcx.query_rows!(context[:golf_db], "SELECT wat FROM players")
    end
  end
  ...
  test "decimal types with spaces near scale and precision" do
    {:ok, db} = Sqlcx.open(":memory:")
    :ok = Sqlcx.exec(db, "CREATE TABLE t (f1 DECIMAL(3,2), f2 DECIMAL(3, 2), f3 DECIMAL( 3 ,2 ))")
    {:ok, []} = Sqlcx.query(db, "INSERT INTO t VALUES (?,?,?)", bind: [Decimal.new("1.23"), Decimal.new("4.56"), Decimal.new("7.89")])
    [row] = Sqlcx.query!(db, "SELECT f1, f2, f3 FROM t")
  ...
  test "it handles datetime, date, time with empty string" do
    {:ok, db} = Sqlcx.open(":memory:")
    :ok = Sqlcx.exec(db, "CREATE TABLE t (a datetime NOT NULL, b date NOT NULL, c time NOT NULL)")
    {:ok, []} = Sqlcx.query(db, "INSERT INTO t VALUES (?, ?, ?)", bind: ["", "", ""])
    {:ok, [row]} = Sqlcx.query(db, "SELECT a, b, c FROM t")
    assert row[:a] == nil
    assert row[:b] == nil
    assert row[:c] == nil
  end
  ...
  test "with_transaction commit" do
    {:ok, db} = Sqlcx.open(":memory:")
    :ok = Sqlcx.exec(db, "create table foo(id integer)")

    Sqlcx.with_transaction(db, fn db ->
      :ok = Sqlcx.exec(db, "insert into foo (id) values (42)")
    end)

    assert Sqlcx.query(db, "select * from foo") == {:ok, [[{:id, 42}]]}
  end
  ...
  test "with_transaction rollback" do
    ...
    assert Sqlcx.query(db, "select * from foo") == {:ok, []}
  end
end
